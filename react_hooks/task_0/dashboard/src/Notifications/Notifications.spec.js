...
test("Does not display drawer elements when displayDrawer is false", () => {
  ...
  expect(getByText("Your notifications")).toBeInTheDocument();
  expect(
    queryByText("Here is the list of notifications")
...
describe("Notifications re-render", () => {
  const list = [
    { id: 1, type: "default", value: "Test 1" },
    { id: 2, type: "urgent", value: "Test_2" },
  ];

  test("does not re-render if notifications length doesn't change", () => {
    const { rerender } = render(<Notifications notifications={list} />);
    const renderSpy = jest.spyOn(Notifications.prototype, "render");

    rerender(<Notifications notifications={[...list]} />);
    expect(renderSpy).not.toHaveBeenCalled();
    renderSpy.mockRestore();
  });

  test("re-render if notifications length changes", () => {
    const { rerender } = render(<Notifications notifications={list} />);
    const renderSpy = jest.spyOn(Notifications.prototype, "render");
    const newList = [...list, { id: 3, type: "default", value: "Test_3" }];
    rerender(<Notifications notifications={newList} />);
